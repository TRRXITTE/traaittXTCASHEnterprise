...
import { WalletBackend } from '@trrxitte/xtcash-wallet-backend';
...
export const directories = [
  `${homedir}/.XTCASHEnterprise`,
  `${homedir}/.XTCASHEnterprise/logs`
];
...
remote.app.setAppUserModelId('wallet.traaitt');

log.debug(`XTCASHEnterprise started...`);
...
eventEmitter.on('sendNotification', function sendNotification(amount) {
  ...
  if (notifications) {
    const notif = new window.Notification('Transaction Received!', {
      body: `${il8n.just_received} ${amount} ${il8n.XTCASH}`
    });
    if (notif) {
      log.debug(`Sent notification: You've just received ${amount} XTCASH.`);
    }
  }
});
...
function handleAbout() {
  remote.shell.openExternal(
    'http://github.com/trrxitte/traaittenterprisexte#readme'
  );
}
...
function handleIssues() {
  remote.shell.openExternal(
    'https://github.com/trrxitte/traaittenterprisexte/issues'
  );
}
