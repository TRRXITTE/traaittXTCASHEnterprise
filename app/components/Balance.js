...
export default class Balance extends Component<Props, State> {
  ...
  switchCurrency = () => {
    ...
    if (displayCurrency === 'fiat') {
      ...
      session.modifyConfig('displayCurrency', 'XTCASH');
      eventEmitter.emit('modifyCurrency', 'XTCASH');
    }
    ReactTooltip.rebuild();
  };
  ...
  render() {
    ...
    if (session.wallet && displayCurrency === 'XTCASH') {
      balanceTooltip =
        `Unlocked: ${session.atomicToHuman(unlockedBalance, true)} ${
          il8n.XTCASH
        }<br>` +
        `Locked: ${session.atomicToHuman(lockedBalance, true)} ${il8n.XTCASH}`;
    } else if (
      session.wallet &&
