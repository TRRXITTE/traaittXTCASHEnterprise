...
export default class Send extends Component<Props, State> {
  ...
  handleAmountChange = (event: any) => {
   ...
   // }
    ...
    const fee = displayCurrency === 'XTCASH' ? 1 : 1 * fiatPrice;
  ...
  createTestTransaction = async () => {
    ...
    if (loopTest && !looping) {
      ...
      this.loopInterval = setInterval(this.createTestTransaction, 100);
    }
  ...
  sendAll = () => {
    ...
    const totalAmount =
      unlockedBalance - 100 - parseInt(session.daemon.feeAmount, 10) <= 0
        ? 0
        : unlockedBalance;
    const enteredAmount =
      unlockedBalance - 100 - parseInt(session.daemon.feeAmount, 10) <= 0
        ? 0
        : totalAmount - 100 - parseInt(session.daemon.feeAmount, 10);
    this.setState({
      totalAmount:
        displayCurrency === 'XTCASH'
    ...
    const addressInput = (
      ...
      >
        <Creatable
          ...
          placeholder="Enter a traaittXTCASH address"
